...
final class Form_Editor {
		...
		wp_localize_script(
			...
			array(
				...
				'i18n'              => array(
					...
					'integrationsNoFields'           => __( 'No fields on this form yet.', 'we-formkit' ),
					'integrationsEditPrompt'         => __( 'Show / edit prompt for this form', 'we-formkit' ),
					'integrationsPromptTypeHint'     => __( 'Type default (from settings):', 'we-formkit' ),
					'integrationsPromptOverrideHint' => __( 'Optional override for this form only. Empty = use the type prompt above.', 'we-formkit' ),
					'integrationsPromptPlaceholder'  => __( 'Leave empty to use the form-type prompt from settings.', 'we-formkit' ),
					...
					'packDragHandle'                 => __( 'Drag template group', 'we-formkit' ),
					'packUngroup'                    => __( 'Ungroup', 'we-formkit' ),
					'packUngrouped'                  => __( 'Template group ungrouped.', 'we-formkit' ),
					'packConfirmDelete'              => __( 'Remove this template group and all its fields?', 'we-formkit' ),
					...
					'showOnFrontend'                 => __( 'Show on frontend', 'we-formkit' ),
					'showOnFrontendHint'             => __( 'Turn off to hide this field on the frontend without deleting it. All settings stay intact.', 'we-formkit' ),
					'parkedBadge'                    => __( 'Hidden', 'we-formkit' ),
					...
					'radioImageDisplay'              => __( 'Display', 'we-formkit' ),
					'radioImageDisplayHint'          => __( 'Image size uses WordPress media sizes (not full resolution). Columns apply from tablet width up; phones stay single-column with the label under the image.', 'we-formkit' ),
					'radioImageSize'                 => __( 'Image size', 'we-formkit' ),
					'imageSizeThumbnail'             => __( 'Thumbnail', 'we-formkit' ),
					'imageSizeMedium'                => __( 'Medium', 'we-formkit' ),
					'radioImageColumns'              => __( 'Columns (desktop)', 'we-formkit' ),
					'radioImageOptionPosition'       => __( 'Option placement', 'we-formkit' ),
					'radioImagePosBelow'             => __( 'Under the image', 'we-formkit' ),
					'radioImagePosBeside'            => __( 'Beside the image', 'we-formkit' ),
					'radioImagePosAbove'             => __( 'Above the image', 'we-formkit' ),
					'radioImageSelectStyle'          => __( 'Selection style', 'we-formkit' ),
					'radioImageSelectRadio'          => __( 'Show radio control', 'we-formkit' ),
					'radioImageSelectFrame'          => __( 'Frame image (hide radio, show checkmark)', 'we-formkit' ),
	...
	private static function sanitize_builder_integrations( array $panels ) {
		...
		foreach ( $panels as $panel ) {
			...
			if ( ! empty( $panel['meta'] ) && is_array( $panel['meta'] ) ) {
				if ( ! empty( $panel['meta']['prompts'] ) && is_array( $panel['meta']['prompts'] ) ) {
					$prompts = array();
					foreach ( $panel['meta']['prompts'] as $kind => $text ) {
						$kind = sanitize_key( (string) $kind );
						if ( '' === $kind ) {
							continue;
						}
						$prompts[ $kind ] = sanitize_textarea_field( (string) $text );
					}
					$clean['meta']['prompts'] = $prompts;
				}
				if ( isset( $panel['meta']['defaultFormKind'] ) ) {
					$clean['meta']['defaultFormKind'] = sanitize_key( (string) $panel['meta']['defaultFormKind'] );
				}
			}
			...
			if ( ! empty( $panel['fields'] ) && is_array( $panel['fields'] ) ) {
				foreach ( $panel['fields'] as $field ) {
					...
					if ( ! in_array( $type, array( 'select', 'textarea', 'text', 'checkboxes', 'prompt_override' ), true ) ) {
						$type = 'text';
					}
					...
					$item = array(
						...
						'choicesSource' => isset( $field['choicesSource'] ) && 'formFields' === $field['choicesSource'] ? 'formFields' : '',
						'kindField'     => isset( $field['kindField'] ) ? sanitize_key( (string) $field['kindField'] ) : '',
						'value'         => $field['value'] ?? '',
						'choices'       => array(),
					);
					...
					if ( 'checkboxes' === $type ) {
						...
						} elseif ( is_string( $item['value'] ) && '' !== $item['value'] ) {
							$parts = preg_split( '/[\s,]+/', $item['value'] );
							if ( ! is_array( $parts ) ) {
								$parts = array();
							}
							$ids = array_values(
						...
						$item['value'] = $ids;
					} elseif ( 'lines' === $item['format'] && is_array( $item['value'] ) ) {
						$lines = array();
						foreach ( $item['value'] as $line ) {
							$line = sanitize_text_field( (string) $line );
							if ( '' !== $line ) {
					...
					if ( in_array( $type, array( 'select', 'checkboxes' ), true ) && ! empty( $field['choices'] ) && is_array( $field['choices'] ) ) {
